{ inputs, config, lib, pkgs, sys, usr, ... }:
let
  lock_cmd = "pidof hyprlock || hyprlock";
  suspend_cmd =
    "pidof steam || systemctl suspend || loginctl suspend"; # fuck nvidia
  ...
  config = mkIf cfg.enable {
    ...
    programs = {
      hyprlock = {
        enable = true;
        extraConfig =
          builtins.replaceStrings [ "~/.config/hypr/hyplock/status.sh" ] [
            "${lib.writeShellScript "hyprlockstatus" builtins.readFile
            ./config/hyprlock/status.sh}"
          ]
