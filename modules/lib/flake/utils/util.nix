...
rec {
  ...
  mkCfg =
    ...
    let
      hostname =
        let
          inherit (inputs.nixpkgs-unstable.lib) last splitString;
        in
        last (splitString "/" path);
      profileCfg = MODULES + /profiles/${getCfgAttr path "sys" "profile"}/config.nix;
    in
      ...
      modules = [
        {
          config = {
            _module.args = {
              pkgs = mkPkgs (getCfgAttr path "sys" "stable") (getCfgAttr path "sys" "system") (
