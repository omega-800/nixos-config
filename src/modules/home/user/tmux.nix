      ...
      + (
        if usr.style then
          ...
          ''
            ...
            #[fg="#${base09}", bg="#${base02}"]  [E]#(ip a | grep -vE '(veth|br-|docker)' | grep -E 'e.*:.*state UP' -A 5 | awk '/inet /{printf $2}')\
            #[fg="#${base0A}", bg="#${base02}"]  [W]#(ip a | grep -vE '(veth|br-|docker)' | grep -E 'wl.*state UP' -A 5 | awk '/inet /{printf $2}')\
            #[fg="#${base0B}", bg="#${base02}"]  [I]#(curl ifconfig.me)\
            #[fg="#${base0C}", bg="#${base02}"]  [V]#(ip a | grep -E 'wg0|ppp0|tun0|home' -A 5 | awk '/inet /{printf $2}' | grep '\.' || echo ''') \
            #[fg="#${base0D}", bg="#${base06}"]" 
          ''
        else
          ""
